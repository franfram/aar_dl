...
Cosas a hacer tal vez: 

- Estandarizar los datos por oveja, para poder contemplar variaciones a nivel de individuo. 



Update August 23 of 2023: We will use this rmd to get a clean dataset to use as the RAW dataset in the paper and maybe upload it to some data bank. The rest of the data wrangling (e.g., chopping data into 5s
segments and prepping for image building will be done in python)
...
# add a tally or count line to see how many files contain X amount of columns
```
...
chopped_datasets <- kept_behaviours[[1]] %>%
  map(
    .x, 
    .f = ~{ .x %>% 
