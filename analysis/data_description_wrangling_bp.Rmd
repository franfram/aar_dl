...
raw_data_dic <- dic_paths %>% 
  ...
  map(
    ~ {
      ...
      # print(paste("colnames of dataframe prior to column select:", names(.x)))
      
      cols_to_select <- if (all(c("Pitch angle", "Roll angle") %in% names(.x))) {
        c("Pitch angle", "Roll angle")
      } else {
        c("Pitch.angle", "Roll.angle")
      }
      
      # cols_list_dec <<- append(cols_list_dec, list(cols_to_select))
     
      # print(paste("TO BE APPENDED COLNAMES", cols_to_select))
      ...
      # 
      # 
      # print(paste("SELECTED COLNAMES: ", c(vars_to_keep_dec, cols_to_select)))
      # 
      # print("TEST")
      # print(names(.x))
...
str(raw_data_dic)
# 
# raw_data_dic %>% map(ncol)

```

#WORKS UP TO THE CELL ABOVE
...
raw_data_sep <- sep_paths %>% 
  ...
  map(
    ~ {
      ...
      # cols_list_sep <<- append(cols_list_sep, list(cols_to_select))
      
      try(select(
        .x,
        c(vars_to_keep_sep, cols_to_select)
      ), silent = TRUE)
...
# raw_data_dic %>% map(ncol)
```
# UPDATE: the one above works but note that there's NAs produced in pitch angle and roll angle. 
...
```{r}
raw_data_sep %>% colnames


raw_data_dic %>% colnames



all("pitch.angle" %in% names(raw_data_sep), FALSE)
?all
```
